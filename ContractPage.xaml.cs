...
public partial class ContractPage : ContentPage
{
	...
	protected override void OnAppearing()
	{
		...
		if (!String.IsNullOrEmpty(contract.Name))
		{
			...
			//typePicker.ItemsSource = App.Database.GetItemsType().Where(x => x.Title == type).ToList();
			//string status = statusPicker.SelectedItem.ToString();
			//statusPicker.ItemsSource = App.Database.GetItemsStatus().Where(y => y.Status == status).ToList();
